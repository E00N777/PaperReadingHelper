...
class LLM:
    ...
    - DeepSeek: V3, R1
    - DashScope-compatible models: Qwen, GLM
    - Claude: 3.5 and 3.7
    """
    ...
    def _get_openai_base_url(self) -> str:
        return os.environ.get("OPENAI_BASE_URL", "https://vpsairobot.com/v1")
    ...
    def _infer_with_openai_responses_model(
        self,
        message: str,
        timeout: int,
        reasoning_effort: str | None = None,
    ) -> str:
        api_key = self._get_api_key("OPENAI_API_KEY").split(":")[0]
    ...
    def _get_api_key(self, *env_names: str) -> str:
        # New code: centralize env-var lookup so the DeepSeek path can reuse
        # the existing REPOAUDIT_API_KEY from the user's shell config.
        for env_name in env_names:
            api_key = os.environ.get(env_name)
            if api_key:
                return api_key
        raise EnvironmentError(
            f"Please set one of the following environment variables: {', '.join(env_names)}."
        )
    ...
    def infer(
        ...
        elif self._is_dashscope_model():
            output = self.infer_with_dashscope_model(message)
    ...
    def _is_dashscope_model(self) -> bool:
        model_name = self.online_model_name.lower()
        return model_name.startswith(("qwen", "glm", "charglm", "zhipu"))
    ...
    def run_with_timeout(self, func, timeout):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                self.logger.print_log(f"Operation timed out after {timeout} seconds")
                return ""
            except Exception as e:
    ...
    def infer_with_dashscope_model(self, message: str) -> str:
        """Infer using DashScope's OpenAI-compatible API."""
        ...
        while tryCnt < 5:
            ...
            self.logger.print_console(
                f"DashScope request attempt {tryCnt}/5 started for {self.online_model_name}."
            )
            try:
                output = self.run_with_timeout(call_api, timeout=180)
                if output:
                    elapsed = time.time() - start_time
                    self.logger.print_console(
                        f"DashScope request attempt {tryCnt}/5 finished in {elapsed:.1f}s."
                    )
                    return output
                elapsed = time.time() - start_time
                self.logger.print_console(
                    f"DashScope request attempt {tryCnt}/5 returned no content after {elapsed:.1f}s."
                )
            except Exception as e:
                elapsed = time.time() - start_time
                self.logger.print_console(
                    f"DashScope request attempt {tryCnt}/5 failed after {elapsed:.1f}s: {e}"
                )
                self.logger.print_log(f"DashScope API error: {e}")
            time.sleep(2)

        return ""

    def infer_with_openai_model(self, message):
        """Infer using an OpenAI-compatible Responses endpoint."""
        return self._infer_with_openai_responses_model(message, timeout=100)

    def infer_with_o3_mini_model(self, message):
        """Infer using the o3-mini model through the same Responses endpoint."""
        return self._infer_with_openai_responses_model(message, timeout=100)
    ...
    def infer_with_deepseek_model(self, message):
        ...
        # New code:
        api_key = self._get_api_key("REPOAUDIT_API_KEY", "DEEPSEEK_API_KEY2")
        model_input = [
            {
                "role": "system",
